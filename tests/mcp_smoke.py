...
print(
    "        tickets =",
    [(t.get("ticket_id"), t.get("status")) for t in r.get("tickets", [])],
)

print("\n" + "=" * 78, "\nGUARDRAILS (must be blocked BEFORE reaching MCP)\n", "=" * 78)
check(
    "P6 RBAC cross-employee read",
    get_leave_balance("EMP-1001", EMP),
    "DENIED",
    "caller != target",
)
check(
    "P3 HITL gate on leave",
    submit_leave_request(
        employee_id=EMP,
        leave_type="vacation",
...
check(
    "G-HCM-3 overlap (backend cannot detect this)",
    submit_leave_request(
        employee_id=EMP,
        leave_type="vacation",
...
check(
    "G-ITSM-1 New->Closed (stricter than backend)",
    update_ticket_status(
        ticket_id="INC0004555", new_status="Closed", caller_id=EMP, user_confirmed=True
    ),
    "DENIED",
)
check(
    "G-HCM-6 address too short",
    update_contact_info(
...
check(
    "G-HCM-7 bad phone",
    update_contact_info(
        employee_id=EMP, phone="nope!!", caller_id=EMP, user_confirmed=True
    ),
    "DENIED",
)
...
r = check(
    "submit_leave_request (confirmed)",
    submit_leave_request(
        employee_id=EMP,
        leave_type="vacation",
...
booked = [
    q
    for q in get_leave_requests(EMP, EMP).get("requests", [])
    if q.get("start_date") == BOOK_START
]
for q in booked:
    check(
        f"cancel_leave_request({q['request_id']})",
        cancel_leave_request(
            employee_id=EMP,
...
if tid:
    ...
    check(
        "add_ticket_comment",
        add_ticket_comment(ticket_id=tid, comment="Smoke test comment.", caller_id=EMP),
        "SUCCESS",
    )
    check(
        "update_ticket_status New->In Progress",
        update_ticket_status(
            ticket_id=tid, new_status="In Progress", caller_id=EMP, user_confirmed=True
        ),
        "SUCCESS",
    )
    check(
        "CLEANUP In Progress->Closed",
        update_ticket_status(
            ticket_id=tid,
            new_status="Closed",
            resolution_notes="Smoke test cleanup.",
            caller_id=EMP,
            user_confirmed=True,
        ),
        "SUCCESS",
